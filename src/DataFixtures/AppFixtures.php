...
class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $indy_img = (new Media())
            ...
            ->setCreationDate(new\ DateTime())
            ->setPostSlug('indy')
            ->setLink('stalefish.jpg');
        $manager->persist($indy_img);
        $indy_res = (new Ressource())
            ->setStatus(1)
            ->setMedia($indy_img)
            ->setType(1);
        ...
        $stalefish_img = (new Media())
            ->setPost($Stalefish)
            ->setCreationDate(new\ DateTime())
            ->setPostSlug('stalefish')
            ->setLink('stalefish.jpg');
        $manager->persist($stalefish_img);
        $stalefish_res = (new Ressource())
            ->setStatus(1)
            ->setMedia($stalefish_img)
            ->setType(1);
        ...
        $tail_grab_img = (new Media())
            ->setPost($Tail_grab)
            ->setCreationDate(new\ DateTime())
            ->setPostSlug('tail_grab')
            ->setLink('tail-grab.jpg');
        $manager->persist($tail_grab_img);
        $tail_grab_res = (new Ressource())
            ->setStatus(1)
            ->setMedia($tail_grab_img)
            ->setType(1);
        ...
        $nose_grab_img = (new Media())
            ->setPost($Nose_grab)
            ->setCreationDate(new\ DateTime())
            ->setPostSlug('nose_grab')
            ->setLink('noseGrab.jpg');
        $manager->persist($nose_grab_img);
        $nose_grab_res = (new Ressource())
            ->setStatus(1)
            ->setMedia($nose_grab_img)
            ->setType(1);
        ...
        $japan_air_img = (new Media())
            ->setPost($Japan_air)
            ->setCreationDate(new\ DateTime())
            ->setPostSlug('japan_air')
            ->setLink('japan.jpg');
        $manager->persist($japan_air_img);
        $japan_air_res = (new Ressource())
            ->setStatus(1)
            ->setMedia($japan_air_img)
            ->setType(1);
        ...
        $seat_belt_img = (new Media())
            ->setPost($Seat_belt)
            ->setCreationDate(new\ DateTime())
            ->setPostSlug('seatbelt')
            ->setLink('seatbelt.jpg');
        $manager->persist($seat_belt_img);
        $seat_belt_res = (new Ressource())
            ->setStatus(1)
            ->setMedia($seat_belt_img)
            ->setType(1);
        ...
        $truck_driver_img = (new Media())
            ->setPost($Truck_driver)
            ->setCreationDate(new\ DateTime())
            ->setPostSlug('truck_driver')
            ->setLink('truck-driver.jpg');
        $manager->persist($truck_driver_img);
        $truck_driver_res = (new Ressource())
            ->setStatus(1)
            ->setMedia($truck_driver_img)
            ->setType(1);
        ...
        $Cent_80 = (new Post())
            ->setTitle('180')
            ->setContent('On désigne par le mot « rotation » uniquement des rotations horizontales ; les rotations verticales sont des flips. Le principe est d\'effectuer une rotation horizontale pendant le saut, puis d\'attérir en position switch ou normal. La nomenclature se base sur le nombre de degrés de rotation effectués. Un 180 désigne un demi-tour, soit 180 degrés d\'angle')
            ->setStatus(1)
            ->setUserId(1)
            ->setCreationDate(new\ DateTime())
        ...
        $cent_80_img = (new Media())
            ->setPost($Cent_80)
            ->setCreationDate(new\ DateTime())
            ->setPostSlug('180')
            ->setLink('180.jpg');
        $manager->persist($cent_80_img);
        $cent_80_res = (new Ressource())
            ->setStatus(1)
            ->setMedia($cent_80_img)
            ->setType(1);
